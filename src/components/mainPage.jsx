...
export default function MainPage(props) {
  ...
  return (
    <Box>
      <InputSection set={{subscriber:setSubscriber,license:setLicense,innerId:setInnerId}} />
      <Box sx={{ paddingTop: 10 }}>
        <Button  loading={loading} sx={{width: "25ch", m: 1 }}  onClick={()=>connectUnitHandleClick()}>
          צמד
        </Button>
      ...
      {serverMsgSuccess !== "" && <Alert variant="outlined" severity="info" sx={{marginTop:5, backgroundColor:"white"}}>
        {serverMsgSuccess}
      </Alert>}
    </Box>
  );
